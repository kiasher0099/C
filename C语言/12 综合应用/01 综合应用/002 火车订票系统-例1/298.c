	...
	while(iWhileListenCount>0)
	{
		...
		m_Server=accept(m_SockServer,(struct sockaddr*)&serveraddrfrom,&len);
			if(m_Server!=INVALID_SOCKET)
			{
				...
				//启动接收消息的线程
				CreateThread(NULL,0,threadproServer,
				(LPVOID)m_Server,0,&nThreadId );
				break;
			}
		printf(".");
		iWhileListenCount--;
	...
	while(1)
	{
		...
		if(strlen(cSendBuffer)>0)//输入消息不能为空
		{
			...
			else
			{
				...
				printf("%s",cShowBuffer);
				fwrite(cShowBuffer ,sizeof(char),strlen(cShowBuffer)
				,ioutfileServer);//将消息写入日志
			}
